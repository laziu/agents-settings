...
- Detect VCS first; support Git, Perforce, SVN, etc.
...
4. Git: commit staged index if present; otherwise stage each group and commit
5. Perforce: create pending changelist per group; open/add/reopen files; never submit
6. SVN: add/delete current-group files, then `svn changelist <name> <paths>`; never commit unless explicit
7. Choose type by behavior impact, not file extension:
   - `feat`: new behavior, capability, route, skill, policy, or output rule
   - `fix`: incorrect, missing, unsafe, or conflicting behavior
   - `refactor`: behavior-preserving restructure, rename, compaction, dedupe
   - `test`: verification code/data only
   - `docs`: human-facing docs only
   - `chore`: packaging, installation, metadata, repo maintenance, operations
